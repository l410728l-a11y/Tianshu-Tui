...
test('GET /sessions/:id/insights aggregates delegation usage and cost', async () => {
  ...
  assert.equal(body.totals.inputTokens, 1_000_000)
  assert.equal(body.totals.outputTokens, 500_000)
  assert.equal(body.totals.cacheReadTokens, 800_000)
  assert.equal(body.totals.cacheWriteTokens, 100_000)
  assert.equal(body.totals.totalTokens, 1_500_000)
