...
const DOMAIN_TONE: Record<string, Array<[string, string]>> = {
  ...
  huagai: [
    // 华盖 — long-run fidelity, day-watch endurance.
    ['准备制定方案', '长路先定守昼锚点'],
    ['开始修改', '托举建设，继续推进'],
  ...
  qiming: [
    // 启明 — 破夜指引，洞察全景。先于建设者一步看清路径。
    ['准备制定方案', '展开全景推演，确定指引方案'],
    ['开始修改', '路径已明，开始引导落地'],
    ['正在修复', '探针已定，直击根因修复'],
    ['代码修改完成', '修改完成，已扫清此路迷雾'],
    ['测试全部通过', '物理事实验证全绿 ✓'],
    ['运行测试验证', '用实测验证指引假设'],
    ['准备交付结果', '全景通透，准备交付'],
    ['收到任务，开始分析', '收到，先用探针看清全貌'],
    ['可能遇到困难', '遇到死循环边缘，重新归纳全局假设'],
    ['接近完成', '即将划破迷雾收尾'],
    ['继续执行中', '晨光同在，持续推进'],
    ['最后验证中', '做最终的全景一致性核验'],
    ['正在分析', '洞察问题全景中'],
  ...
  changgeng: [
    ...
    ['运行测试验证', '静心验证每一处细节'],
    ['准备交付结果', '终局成全，笃定交付'],
    ['收到任务，开始分析', '收到，不疾不徐全盘推演'],
    ['可能遇到困难', '遇到长夜关卡，静心梳理再战'],
    ['接近完成', '即将安稳收尾'],
    ['继续执行中', '长庚同在，静心推进'],
    ['最后验证中', '做最后一英里的沉稳核验'],
    ['正在分析', '沉静推演中'],
