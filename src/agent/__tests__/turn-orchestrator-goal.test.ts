 ...
 * the (otherwise private) judgeGoalCompletion directly with minimal stub deps —
 * exercising the full run loop would require mocking the entire turn pipeline.
 */
...
function judge(orch: TurnOrchestrator, tracker: GoalTracker, signal?: AbortSignal): Promise<Decision> {
  return (orch as unknown as { judgeGoalCompletion: (t: GoalTracker, s: AbortSignal | undefined) => Promise<Decision> })
    .judgeGoalCompletion(tracker, signal)
}
...
describe('TurnOrchestrator.judgeGoalCompletion', () => {
  it('accepts (legacy) without recording a judge run when judge is disabled', async () => {
    const orch = makeOrchestrator({ judgeDeps: undefined })
  ...
  it('accepts when the judge verifies', async () => {
    const orch = makeOrchestrator({ judgeDeps: { spawnJudge: async () => verdictRun('verified', 'all good') } })
    const tracker = makeTracker()
    const decision = await judge(orch, tracker, undefined)
    assert.equal(decision.action, 'accept')
  ...
  it('accepts with an unverified warning on inconclusive (fail-open)', async () => {
    const orch = makeOrchestrator({ judgeDeps: {} }) // no spawnJudge → inconclusive
    const tracker = makeTracker()
    const decision = await judge(orch, tracker, undefined)
    assert.equal(decision.action, 'accept')
  ...
  it('reaches accept after exhausting the cap across repeated rejections', async () => {
    ...
    const tracker = makeTracker(3)
    assert.equal((await judge(orch, tracker)).action, 'continue') // run 1
    assert.equal((await judge(orch, tracker)).action, 'continue') // run 2
    assert.equal((await judge(orch, tracker)).action, 'accept')   // run 3 = cap
