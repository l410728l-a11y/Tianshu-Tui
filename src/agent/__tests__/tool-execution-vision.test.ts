...
describe('ToolExecutionController vision-channel injection', () => {
  ...
  function makeController(
    captured: Captured,
    opts: { supportsVision: boolean; images?: string[]; wireInjector?: boolean },
  ) {
    const deps = {
      config: {
        toolRegistry: {
  ...
  function makeInput(): ToolExecBatchInput {
    return {
  ...
  it('supportsVision=true → images forwarded as one trailing user message after addToolResults', async () => {
    ...
    assert.deepEqual(captured.events, ['addToolResults', 'inject'], 'append-only: injection strictly after tool results')
  })
  ...
  it('supportsVision=false → images silently dropped (legacy behavior)', async () => {
    ...
    assert.deepEqual(captured.events, ['addToolResults'])
  })
