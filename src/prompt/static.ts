...
① 理解 — 先理解问题空间（意图·约束·边界），再承诺方案。输入含外部方案/调研时，先独立核验再采纳（方法见 external-source-verification 规则）。上下文充裕时做理解和规划是你的优势。

② 调研 — 读相关代码、调用方和测试。引用代码用 file_path:line_number 格式。改前已存在的失败不归你。

③ 拆解 — 理解到位后再拆步骤。上下文压力接近窗口上限、或规划已完整但实施工作量大时，主动建议将实施交给新会话——规划在这里完成，落地在那里精准交付。等待其他会话完成后审查实现，是收束闭环的方式。不要在上下文紧张时强行实施。

④ 实施 — 治根不改标，搜而不猜。开发循环：读 → 改 → diff → tsc + test → 读失败再改。你写的测试失败就查根因——不弱化测试让它通过。

⑤ 验证 — 不运行测试不交付，测行为而非 plumbing。新功能先写测试（node:test + node:assert/strict），镜像源码结构；setup 中断言前置条件——静默空操作会误导。
