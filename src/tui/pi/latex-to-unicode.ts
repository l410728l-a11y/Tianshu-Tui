// Terminal capability injected by the host (see `setMathColorTrueColor`).
// Defaults to ANSI-256 (conservative — universal). Hosts that have detected
// 24-bit color support (Kitty/iTerm2/WezTerm/Ghostty/…) call
// `setMathColorTrueColor(true)` to enable the richer `ansi-16m` path.
let mathColorTrueColor = false;
 ...
 */
export function setMathColorTrueColor(enabled: boolean): void {
	mathColorTrueColor = enabled;
}

// ── Bun.color replacement (Node-compatible) ──────────────────────────
// The upstream module uses Bun's built-in CSS color parser. Tianshu targets
// Node 22, so we provide a minimal equivalent covering only the formats LaTeX
// color commands produce: named colors (via LATEX_NAMED_COLORS), `#rgb` /
// `#rrggbb` / `#rrggbbaa` hex, and `rgb()/rgba()` functional notation.
// `Bun.color(spec, "css")` → validate (echo spec or null);
// `Bun.color(spec, "{rgb}")` → {r,g,b} or null;
// `Bun.color(spec, "ansi-256"|"ansi-16m")` → ANSI escape or null.

interface ParsedRgb { r: number; g: number; b: number; }

function parseCssColorToRgb(spec: string): ParsedRgb | null {
	const s = spec.trim();
...
function rgbToAnsi256({ r, g, b }: ParsedRgb): string {
	if (r === g && g === b) {
...
function rgbToAnsi16m({ r, g, b }: ParsedRgb): string {
	return `\x1b[38;2;${r};${g};${b}m`;
}

function bunColorShim(spec: string, format: "css"): string | null;
function bunColorShim(spec: string, format: "{rgb}"): ParsedRgb | null;
function bunColorShim(spec: string, format: AnsiColorFormat): string | null;
function bunColorShim(spec: string, format: "css" | "{rgb}" | AnsiColorFormat): string | ParsedRgb | null {
...
function colorFormat(): AnsiColorFormat {
	return mathColorTrueColor ? "ansi-16m" : "ansi-256";
}
