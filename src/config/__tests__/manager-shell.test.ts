...
describe('shell (Git Bash path) config', () => {
  ...
  it('getShellConfig returns empty string when unset', () => {
    assert.deepEqual(getShellConfig(), { gitBashPath: '' })
  })

  it('persists a custom path and round-trips through loadConfig', () => {
    const r = setShellConfig({ gitBashPath: 'C:\\custom\\Git\\bin\\bash.exe' })
    assert.deepEqual(r, { gitBashPath: 'C:\\custom\\Git\\bin\\bash.exe' })
    assert.equal(loadConfig().env.gitBashPath, 'C:\\custom\\Git\\bin\\bash.exe')
    assert.deepEqual(getShellConfig(), { gitBashPath: 'C:\\custom\\Git\\bin\\bash.exe' })
  })
  ...
  it('an empty/whitespace value clears the override', () => {
    ...
    assert.deepEqual(cleared, { gitBashPath: '' })
